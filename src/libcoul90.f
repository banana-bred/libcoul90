...
module libcoul90
  ...
  interface
    module subroutine coul90f(X, ETA_IN, XLMIN, LRANGE, FC, FCP, KFN, IFAIL)
      use iso_fortran_env, only: dp => real64
      integer, intent(in)  :: KFN
      integer, intent(in)  :: LRANGE
      integer, intent(out) :: IFAIL
...
! ================================================================================================================================ !
  ...
  !------------------------------------------------------------------------------------------------------------------------------- !
  subroutine coul90f_wrapper(xlmin, nl, eta, x, f, fp, kfn)
    !! A wrapper routine to call coul90f used to only calculate the functions held in the
    !! arrays F and FP. The integers MODE and KFN
    !! must be set as follows to obtain the desired output from
    !! coul90f :
    !!
    !! The contents of F and FP are controlled by KFN.
    !! ________________________________
    !!| ARRAY |          KFN          |
    !!|       |   0   |   1   |   2   |
    !!|_______|_______|_______|_______|
    !!|   F   | F_λ   |  j_λ  |  J_λ  |
    !! --------------------------------
    !! FP is the derivative of the function represented by F
    use iso_fortran_env, only: dp => real64
    real(dp),  intent(in)   :: xlmin
      !! The minimum value of λ
    integer,  intent(in)    :: nl
    ...
    integer :: lrange
    lrange = nl - 1
    call coul90f(x, eta, xlmin, lrange, f, fp, kfn, ifail)
    select case(ifail)
    case(0)
      continue
    case(1)
  ...
  end subroutine coul90f_wrapper
